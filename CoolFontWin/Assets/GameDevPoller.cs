...
namespace SuncomControllerMerge
{
    public class GameDevPoller
    {
        ...
        public bool ValidateDeviceExistance()
        {
            ...
            foreach (Object obj in gameControllerList)
            {
        ...
        bool DetectDevice(DeviceInstance deviceInstance, string name, ref Device dev, Label lbl)
        {
            if (deviceInstance.ProductName.ToLower().StartsWith(name))
            {
                // create a device from this controller.
                dev = new Device(deviceInstance.InstanceGuid);
                dev.SetCooperativeLevel(mainForm,
        ...
        public bool Poll()
        {
            ...
            try
            {
                ...
                state = throttleDevice.CurrentJoystickState;
